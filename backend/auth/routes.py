...
def verify():
    ...
    GET  — check the token is valid and show a confirm button (safe for email scanners).
    POST — consume the token and start a session.
    """
    from backend.auth.service import peek_magic_link
    token = request.args.get('token') or request.form.get('token')

    if request.method == 'GET':
        # Only peek — do NOT consume the token
        user = peek_magic_link(token)
        if not user:
            flash('This link is invalid, expired, or has already been used. Request a new one below.', 'error')
            return redirect(url_for('auth.login'))
        return render_template('auth/verify.html', token=token, user=user)

    # POST — now consume the token
...
@auth_bp.route('/auth/change-name', methods=['POST'])
def change_name_route():
    if not session.get('user_id'):
        return redirect(url_for('auth.login'))
    ...
    ok = change_name(session['user_id'], name)
    if ok:
        session['user_name'] = name
        flash(f'Name updated to {name}!')
    else:
